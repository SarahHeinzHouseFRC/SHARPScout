...
if (isset($_POST['type']))
{
    $postedType = $_POST['type'];
}
else
{
    $postedType = "versioncheck";
}

echo $postedType;

if ($postedType == 'passConfirm' && $_POST['password'] == $pass)
{
    header($_SERVER['SERVER_PROTOCOL'] . ' 200 OK');
    echo 'success';
}
elseif ($postedType == 'versioncheck')
{
    header($_SERVER['SERVER_PROTOCOL'] . ' 200 OK');
    echo $ver;
}
elseif ($_POST['password'] == $pass)
{
    ...
    else if ($postedType == 'match')
    {
        ...
        $result       = mysql_query("SELECT id FROM fact_match_data WHERE event_id=" . $event_id . " AND match_id=" . $match_id . " AND team_id=" . $team_id);
        $row          = mysql_fetch_array($result);
        $match_row_id = $row["id"];
        
        if (mysql_num_rows($result) == 0)
        {
            $query = "INSERT INTO fact_match_data(event_id,match_id,team_id,auto_high,auto_high_hot,auto_low,auto_low_hot,high,low," . "auto_mobile,auto_goalie,num_cycles,foul,tech_foul,tip_over,yellow_card,red_card,notes,invalid) VALUES(" . $event_id . "," . $match_id . "," . $team_id . "," . $auto_high . "," . $auto_high_hot . "," . $auto_low . "," . $auto_low_hot . "," . $high . "," . $low . "," . $auto_mobile . "," . $auto_goalie . "," . $num_cycles . "," . $foul . "," . $tech_foul . "," . $tip_over . "," . $yellow_card . "," . $red_card . ",'" . $notes . "',0);";
            
            $success = mysql_query($query);
        }
        else
        {
            $query = "UPDATE fact_match_data SET " . "event_id=" . $event_id . ",match_id=" . $match_id . ",team_id=" . $team_id . ",auto_high=" . $auto_high . ",auto_high_hot=" . $auto_high_hot . ",auto_low=" . $auto_low . ",auto_low_hot=" . $auto_low_hot . ",high=" . $high . ",low=" . $low . ",auto_mobile=" . $auto_mobile . ",auto_goalie=" . $auto_goalie . ",num_cycles=" . $num_cycles . ",foul=" . $foul . ",tech_foul=" . $tech_foul . ",tip_over=" . $tip_over . ",yellow_card=" . $yellow_card . ",red_card=" . $red_card . ",notes='" . $notes . "',invalid=0" . " WHERE id=" . $match_row_id;
        ...
        if ($success)
        {
            $result = mysql_query("SELECT id, timestamp FROM fact_match_data WHERE event_id=" . $event_id . " AND match_id=" . $match_id . " AND team_id=" . $team_id);
            $row    = mysql_fetch_array($result);
    ...
    else if ($postedType == 'cycle')
    {
        ...
        $result       = mysql_query("SELECT id FROM fact_cycle_data WHERE event_id=" . $event_id . " AND match_id=" . $match_id . " AND team_id=" . $team_id . " AND cycle_num=" . $cycle_num);
        $row          = mysql_fetch_array($result);
        $match_row_id = $row["id"];
        
        if (mysql_num_rows($result) == 0)
        {
            $query = "INSERT INTO fact_cycle_data(event_id,match_id,team_id,cycle_num,near_poss,white_poss,far_poss," . "truss,catch,high,low,assists,invalid) VALUES(" . $event_id . "," . $match_id . "," . $team_id . "," . $cycle_num . "," . $near_poss . "," . $white_poss . "," . $far_poss . "," . $truss . "," . $catch . "," . $high . "," . $low . "," . $assists . ",0);";
            
            $success = mysql_query($query);
        }
        else
        {
            $query = "UPDATE fact_cycle_data SET " . "event_id=" . $event_id . ",match_id=" . $match_id . ",team_id=" . $team_id . ",cycle_num=" . $cycle_num . ",near_poss=" . $near_poss . ",white_poss=" . $white_poss . ",far_poss=" . $far_poss . ",truss=" . $truss . ",catch=" . $catch . ",high=" . $high . ",low=" . $low . ",assists=" . $assists . ",invalid=0" . " WHERE id=" . $match_row_id;
            
            $success = mysql_query($query);
        }
        if ($success)
        {
            $result = mysql_query("SELECT id, timestamp FROM fact_cycle_data WHERE event_id=" . $event_id . " AND match_id=" . $match_id . " AND team_id=" . $team_id . " AND cycle_num=" . $cycle_num);
            $row    = mysql_fetch_array($result);
            $resp   = $row["id"] . "," . strtotime($row["timestamp"]);
        }
        else
    ...
    else if ($postedType == 'pits')
    {
        ...
        {
            $success = false;
            $query   = "INSERT INTO scout_pit_data(team_id,configuration_id,wheel_type_id,wheel_base_id,autonomous_mode,scout_comments," . "auto_high,auto_low,auto_hot,auto_mobile,auto_goalie,truss,catch,active_control,launch_ball,score_high,score_low,max_height,invalid) " . "VALUES(" . $team_id . "," . $configuration_id . "," . $wheel_type_id . "," . $wheel_base_id . "," . $autonomous_mode . ",'" . $scout_comments . "'," . $auto_high . "," . $auto_low . "," . $auto_hot . "," . $auto_mobile . "," . $auto_goalie . "," . $truss . "," . $catch . "," . $active_control . "," . $launch_ball . "," . $score_high . "," . $score_low . "," . $max_height . ",0);";
